...
| Derived in-app docs | `app/docs/` | Copies of editorial Markdown served by GitHub Pages. Regenerated by `scripts/sync_docs.py`. **Never edit by hand.** |
...
`python3 scripts/check_md_json.py --quiet`,
`python3 scripts/sync_docs.py --check`, and
...
2. **`--check` mode** on `scripts/sync_docs.py` — editorial Markdown copies in `app/docs/` match the sources.
3. **CI gate** — the checks run in the non-regression workflow.
4. **Unique sentinels** around embedded blocks — structural anchors for sync/extract.
5. **Embedded-fallback test** — aborts `fetch` of the JSON files and asserts the same
   species IDs, event IDs, species certainty fields, and event certainty fields.
6. **Test runner exit status** — `node tests/run-all.js` exits non-zero on failure
   (local and CI).
7. **DOI verification** — `python scripts/check_dois.py` resolves every DOI cited in
...
8. **Markdown↔JSON correspondence** — `python scripts/check_md_json.py` requires
   every catalogue `@id` to have a Markdown `id`, JSON DOIs to be a subset of the
   paired Markdown DOIs, event `debateLevel` / `evidenceType` to match the
   Markdown tokens, and species taxonomy / behaviour / pigmentation debate and
   evidence tokens to match the morphology catalogue map. `UNASSESSED` on an
   event is explicit “not yet synthesised”, not consensus.
