...
4. If editorial Markdown changed, copy it into the app for GitHub Pages: `python scripts/sync_docs.py`
5. Verify correspondence and the citation you added: `python scripts/check_md_json.py` then `python scripts/check_dois.py`
6. Run `node tests/run-all.js` to verify nothing is broken

Many JSON-LD narrative fields carry both `en` and `fr`. The **page chrome is English** (`<html lang="en" translate="yes">`) so the browser’s **Translate this page** can localise menus. A burger **Catalogue language** control (`ho_catalogue_lang`, or `?lang=fr` / `?lang=en`) switches only the scientific JSON strings. A French browser selects that French catalogue automatically. Bundled French catalogue text is marked `translate="no"` so a page translator does not rewrite the curated debates. Continent labels are English and translatable; country names use the browser’s `Intl.DisplayNames`.
...
The menu chrome is **English**. Use the browser’s **Translate this page** for any other UI language. Scientific catalogue strings still ship as `{en,fr}` in JSON; the burger **Catalogue language** control switches them (`localStorage` key `ho_catalogue_lang`, or `?lang=fr` / `?lang=en`). A French browser selects Français automatically. Editorial Markdown is English and is also readable inside the app (Sources drawer), so a page translator can localise it.
...
- Translators: improve the `{en,fr}` catalogue strings in `app/data/*.json`, or rely on the browser’s **Translate this page** for the English chrome (see [CONTRIBUTING.md](CONTRIBUTING.md))
